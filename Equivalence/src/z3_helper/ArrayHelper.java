...
public class ArrayHelper {
	public static Expr z3Object(ArrayRef v, PathCoverter theCoverter,
			PathHelper pathHelper, Map<String, Expr> latestLocal,boolean IfAssignLeft,int nodeIndex) {
		    InterpolationContext ictx = theCoverter.getIctx();
		    Value base=v.getBase();
			Value index=v.getIndex();
			Expr baseZ3=pathHelper.CovertValue(base, false, nodeIndex);
			Expr indexZ3=pathHelper.CovertValue(index, false, 0);
			if(IfAssignLeft){
				return baseZ3;
			}
			else{
				Expr select=ictx.mkSelect((ArrayExpr)baseZ3, indexZ3);
				return select;
			}
	}
